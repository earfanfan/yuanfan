...
dt.DT1 <- dt[, .( # 部分参数含义见备注（按P键）。
  '面积图' = spark_html(value1, type = "line",width=100),
  '柱状图' = spark_html(value1, type = "bar",width=100),
  '折线图' = spark_html(
    value1,
    type = "line",
    lineColor = "red",
...
dt <- as.data.table(data)
spark_html <- function(...) {
  as.character(htmltools::as.tags(sparkline(..., height = 100)))}

dt.DT1 <- dt[, .( # 部分参数含义见备注（按P键）。
  '面积图' = spark_html(value1, type = "line", width=100),
  '柱状图' = spark_html(value1, type = "bar", width=100),
  '折线图' = spark_html(value1, type = "line", lineColor = "red", fillColor = FALSE, width=100),
  '柱状图2' = spark_html(value1, type = "bar", barColor = "green", width=100),
  '箱图' = spark_html(value1, type = "box", width=100) ), keyby = .(type2)]
...
dt.DT2 <- dt[, .('饼图' = spark_html(value1, type = "pie", sliceColors = c("red", "green"), 
                  offset = 90, width=100)),  keyby = .(type1)]
