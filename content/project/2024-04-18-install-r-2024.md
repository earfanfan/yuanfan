...
```bash
sudo yum install libicu-devel readline-devel libXt-devel bzip2-devel lzma xz-devel libcurl-devel libxml2-devel
```
...
2025年底安装最新 xgboost 踩新坑： `gcc -v`看到是 GCC 8.5，但是系统实际加载的 libstdc++ 是旧版本。这次折腾的服务器已经安装了别的应用，不能随意瞎折腾，改为安装 xgboost 旧版本。
