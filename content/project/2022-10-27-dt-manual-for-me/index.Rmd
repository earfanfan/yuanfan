...
datatable(
  data,
  caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left; font-size: 20px; font-weight: bold;','表1：一个表格的标题'))
```
...
datatable(
  data,
  caption = htmltools::tags$caption(style = 'caption-side: bottom; text-align: left; color: red;',
                                    '注1：一个表格的脚注'))
```
...
+ 初始化过滤条件。如下`search = '25 s'`所匹配的结果是过滤出来的每一行都同时包括“25”和“s”。

```{r}
datatable(data,
          options = list(search = list(search = '25 s' #初始化过滤条件
          )))
```
...
```{r}
datatable(data, options = list(searchHighlight = TRUE, search = list(search = '25 s')))
```
...
bscols(list(
  ...
  # htmltools::span("文字1",summarywidget::summarywidget(shared_data, statistic='mean', column='value', digits=1),"文字2"),
  DT::datatable(
    shared_data,
    width = '100%',
    rownames = FALSE,
